...
http://git.musl-libc.org/cgit/musl/tree/src/math/expm1.c?h=v1.1.16
*/
	...
	public static partial class Functions
	{
		...
		public static double ExpMinus1(double x)
		{
			...
			{  /* if  |x| > 0.5 ln2 */
				...
				{
					k = (int)(invln2 * x + (u.SignBit ? -0.5 : 0.5));
					t = k;
			...
			u.Bits = (ulong)(0x3ff + k) << 52;  /* 2^k */
			twopk = u.Value;
			if (k < 0 || k > 56)
			{  /* suffice to return exp(x)-1 */
				y = x - e + 1.0;
				if (k == 1024)
					y = y * 2.0 * Constants.Pow2P1023;
				else
					y = y * twopk;
				return y - 1.0;
			}
			u.Bits = (ulong)(0x3ff - k) << 52;  /* 2^-k */
			if (k < 20)
				y = (x - e + (1 - u.Value)) * twopk;
			else
				y = (x - (e + u.Value) + 1) * twopk;
			return y;
		}
	}
}
